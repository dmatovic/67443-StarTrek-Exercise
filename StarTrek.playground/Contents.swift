...
print(Float(femaleOfficers.count) / Float(humanCount))



// Q2: How many first officers are human?
print(firstOfficers.keys.filter{humanOfficers.contains($0)}.count)
...
let allCapitains: [Officer] = captains.map{ Officer(name: $0, ship: $1)}
print(allCapitains.map{$0.name})



// Q5: Alphabetize the previous list by the captain's name
let allCaptainsAplpha = captains.map{Officer(name: $0, ship: $1)}.sorted(by: {$0 < $1})
print(allCaptainsAplpha.map{$0.name})
...
let vulcanFirstOfficers: [Officer] = allFirstOfficers.filter{vulcanOfficers.contains($0.name)}
...
let vulcanFirstOfficersLess5 = vulcanFirstOfficers.filter{$0.name.count < 5}.count
print(vulcanFirstOfficersLess5)
